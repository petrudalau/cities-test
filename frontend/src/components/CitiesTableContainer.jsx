...
export const CitiesTableContainer = ({
  cityListDispatch,
  updateDispatch,
  cityToEdit,
  loading,
  ...
  }, [cityListDispatch, pageSize, page, nameFilter]);
  useEffect(() => {
    countCities(cityListDispatch, nameFilter);
  }, [cityListDispatch, nameFilter]);
  const editorClosed = useCallback(() => {
    editCity(updateDispatch, null);
    loadCityList(cityListDispatch, pageSize, page, nameFilter);
  }, [cityListDispatch, updateDispatch, pageSize, page, nameFilter]);
  const editCityHandler = useCallback((city) => {
    editCity(updateDispatch, city);
  }, [updateDispatch]);
  const updateCityHandler = useCallback((city) => {
    updateCity(updateDispatch, city);
  }, [updateDispatch]);
  if (!loading && error) {
  ...
  return (<React.Fragment>
    {cityToEdit &&
        <EditCityComponent city={cityToEdit} closeEditor={editorClosed}
          updateCityHandler={updateCityHandler} saveStatus={saveStatus}/>}
      ...
      <tbody>
        ...
        <tr>
          <td>
            ...
            {(cities && !cityToEdit) &&
                <CitiesTableComponent columns={columnToDisplay} rows={cities} editCity={editCityHandler}/>}
          </td>
        </tr>
